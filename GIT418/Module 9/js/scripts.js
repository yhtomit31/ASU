 ...
 *
 **/
function createEx(event) { // Prevent default submission execution
...
function addToExTable(ex) { // Initialize and assign variables for table body and new row
    var exTable = document.getElementById("exTable").getElementsByTagName("tbody")[0];
    ...
    **/
        ...
        if (data === "exDislikes") {
            for (var x = 0; x <= ex[data].length - 1; x++) {
                if (x === ex[data].length - 1) { // Template Literal is string allowing expressions. Only used .toString() for ease of grading. Probably unnecessary.
                    newCell.innerHTML += `${
                        ex[data][x]
                    } `.toString();
                } else { // Template Literal is string allowing expressions. Only used .toString() for ease of grading. Probably unnecessary.
                    newCell.innerHTML += `${
                        ex[data][x]
                    }, `.toString();
                }
